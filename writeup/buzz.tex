...
\begin{figure}[h]
  \centering
    ...
    \captionof{figure}{\setstretch{1.3} \textbf{Framework demonstrating the effect of the climate on the different measures of turnovers.} Environmental changes in temperature or precipitation can lead to modifications in plant phenology, which may result in an elevated plant turnover. A higher plant turnover can lead to a higher dissimilarity between interaction networks due to species turnover, resulting in a higher interaction turnover.}
    \label{fig: flowchart}
\end{figure}
\vspace{-0.3cm}
...
\newpage
\vspace{5cm}
At both sites, correlation between interaction turnover, $\beta_{int}$, and plant turnover, $\beta_{Plant}$, did not occur by chance (random rewiring), implying that $\beta_{Plant}$ has a significant and strong positive correlation with interaction turnover due to non-random rewiring wherein certain bees preferentially visit certain, newly-available plants (BBG: $r_{s}$=0.822, $p$=0.00008; IBGE: $r_{s}$=0.773, $p$=0.0118)~(\autoref{fig: plantturnover}). Hence, $\beta_{Plant}$ drives $\beta_{int}$ and is a strong predictor of $\beta_{int}$.\\
\vspace*{\fill}
\begin{figure}[H]
  \centering
...
($r_{s}$: Spearman's correlation coefficient; $p_{s}$: p-value of Spearman's test; p-value: value generated using 100000 randomised networks for each month)}
