...
"""Calculating yearly turnover in Cerrado"""
...
print 'startofyears' + str(startofyears)
print len(startofyears)

# for every loop, startindex is where year 1 starts, nextindex is where year 2 starts, thereafter compare year 1 and year 2
# calculate a, similarity bet two consecutive years, and b & c, unique pairs in each year.
...
rangeofyears = range(0, len(startofyears)- noofyears*2, noofyears)

print rangeofyears

for x in rangeofyears:
  startindex = startofyears[x]
  nextindex = startofyears[x + noofyears]
  nextnextindex = startofyears[x + noofyears*2]
  a = 0
  b = 0
  c = 0
...
pl.title(' yearly turnover rate Cerrado')

pl.savefig('../results/yearlyCerrado.pdf')
# pl.plot(x, y, options)
# ro : scatterplots
