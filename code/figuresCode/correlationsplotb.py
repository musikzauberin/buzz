...
h = open('../results/BivariatePlots/OldCerrado/monthavg&turnover/Turnover&ClimateAvg(Old).csv','rb')
data = csv.reader(h)

h1 = open('../results/BivariatePlots/OldCerrado/monthdiff&turnover/Turnover&ClimateDiff(Old).csv','rb')
data1 = csv.reader(h1)
...
for i in range(0, len(ystarts), 2):
  ...
  for j in range(ystarts[i],ystarts[i+1]):
    for dotmeasure in dotmeasures:
      if xmeasures[j] == dotmeasure:
        ...
        pl.text(i/2+0.1, dryrs[j], dotlabels[h], size = 10)
  ...
  for j in range(ystarts[i],ystarts[i+1]):
    for dotmeasure in dotmeasures:
      if xmeasures[j] == dotmeasure:
        h = dotmeasures.index(dotmeasure)
        pl.text(i/2+0.1, wetrs[j], dotlabels[h], size = 10)
  ...
  for j in range(ystarts[i],ystarts[i+1]):
    for dotmeasure in dotmeasures:
      if xmeasures[j] == dotmeasure:
        h = dotmeasures.index(dotmeasure)
        pl.text(i/2+0.1, allrs[j], dotlabels[h], size = 10)
...
titles = 'Dry Season(Old), Wet Season(Old), All(Old)'
titles = titles.split(', ')
...
for nrows, ncols, plot_number in plotdimensions:
  ...
  # x labels
  major_ticks = np.arange(0, len(ystarts)/2)
...
# save plot and show
pl.tight_layout()
plotname = 'SpecTurnover&AvgClimate(Old)'
