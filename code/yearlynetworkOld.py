...
def findstartindex(values, timescale_str):
  ...
  if timescale_str == 'unique':
    for i in range(1, datalen):
      if values[i] == '6' and values[i] != values[i-1]:
        startofunq.append(i)
    print 'unique: ' 
    startofunq.pop(-1)
...
appearmax = 0
beeplants1 = beeplants[startofunq[0]:startofunq[1]]
beeplants2 = beeplants[startofunq[1]:startofunq[2]]

# define colours, alpha and widths for lines dependent on number of int appearances
colours2 = [colours[i] for i in [0, 2]]
alphanos = [0.2, 1.0]
widths = [0.5, 1.0]
...
for i in range(len(uniqueints)):
  if beeplants.count(uniqueints[i]) > 1:
    if uniqueints[i] in beeplants1 and uniqueints[i] in beeplants2:
      appearno = 2
    else:
      appearno = 1
  else: 
...
labelsstr1 = '6/1995-5/1996 or 6/1996-6/1997, 6/1995-5/1996 and 6/1996-6/1997'
