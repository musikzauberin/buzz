...
def plotturnovers(a, b, drya, dryb, weta, wetb, xlabel_str, ylabel_str):
  ...
  # correlation text
  rc('text', usetex=True)
  pl.text(xmax - 0.2, ymin, r' \underline{Spearman`s coefficient}' + '\n Dry Season: ' + str(round(dryr, 5)) + \
  ...
  plotpath = '../results/ClimaPlots/OldCerradoClima/turnovers/' + plotname + '.pdf'
  pl.savefig(plotpath)
  pl.close()
...
def writenewdata(filename_str, headers, values):
  ...
  pathname = '../results/ClimaPlots/OldCerradoClima/turnovers/' + filename_str + '.csv'
  g = open(pathname, 'wb')
...
for i in range(len(turnoverset)):
  for xturnover in turnoverxset[i]:
    if xturnover == turnoverset[0]:
      h = 0
    if xturnover == turnoverset[1]:
      h = 1
    if xturnover == turnoverset[2]:
      h = 2
    if xturnover == turnoverset[3]:
      h = 3
    if xturnover == turnoverset[4]:
      h = 4
    if xturnover == turnoverset[5]:
      h = 5
...
writenewdata('TurnoverCoefficients', newheaders, newvalues)
