...
# correlation text
rc('text', usetex=True)
pl.text(xmax - 0.15, ymin,  r'$r_{s}$ = ' + str(round(allr, 3)) + '\n' + r'$p$ = 0.031', size = 12)
...
# correlation text
rc('text', usetex=True)
pl.text(0.41, 0.01,  r'$r_{s}$ = ' + str(round(allr, 3)) + '\n' + r'$p$ = 0.176', size = 12)
...
# correlation text
rc('text', usetex=True)
pl.text(xmax - 0.15, 0.01,  r'$r_{s}$ = ' + str(round(allr, 3)) + '\n' + r'$p$ = 0.177', size = 12)
...
# grid
pl.suptitle('Turnover measures of BBG networks', size=16)
pl.grid(True)

pl.tight_layout()
pl.subplots_adjust(top=0.88)
# save plot and show
plotpath = '../turnoversbivariate(old).pdf'
pl.savefig(plotpath)
pl.show()