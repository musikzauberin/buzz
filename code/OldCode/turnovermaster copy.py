...
def calc_intturnover(months, days, years, bees, plants, noofmonths, startofmonths):
  'Calculate interaction turnover'
  monthsindices = startofmonths[0::noofmonths] # pick the appropriate numbers from startofmonths to get the list of startindex and nextindex
...
def calc_speciesturnover(months, days, years, bees, plants, noofmonths, startofmonths):
  'Calculate species, plant and bee turnover (by first removing plant and bee duplicates)'
  monthsindices = startofmonths[0::noofmonths] # pick the appropriate numbers from startofmonths to get the list of startindex and nextindex
  'deletes plant and bee duplicates within noofmonths intervals'
  pmonths = months
  pplants = plants
  for m in reversed(range(len(monthsindices) - 1)): # doing it in reverse allows me to delete directly from the list without any maddening indexing issues!
    startindex = monthsindices[m]
    ...
    for i in reversed(range(startindex, nextindex)):
      for j in reversed(range(startindex, i - 1)):
        # if bbees[i] == bbees[j]:
        #   bmonths.pop(i) # remove duplicate element
        #   bbees.pop(i)
        #   break
        if pplants[i] == pplants[j]:
          print (i, months[i], days[i], plants[i]), \
                'matches with', (j, months[j], days[j], plants[j])
          pmonths.pop(i) # remove duplicate element
          pplants.pop(i)
          break
  print pmonths, pplants
  plantstartofmonths = findstartindex(pmonths, 'months')
  plantmonthsindices = plantstartofmonths[0::noofmonths]
  print plantmonthsindices
  alist = [0] * (len(plantmonthsindices) - 2)
  blist = [0] * (len(plantmonthsindices) - 2)
  clist = [0] * (len(plantmonthsindices) - 2)
  bints = [0] * (len(plantmonthsindices) - 2)
  for m in range(len(plantmonthsindices) - 2):
    startindex = plantmonthsindices[m]
    nextindex = plantmonthsindices[m + 1]
    nextnextindex = plantmonthsindices[m + 2]
    for i in range(startindex, nextindex):
      for j in range(nextindex, nextnextindex):
        if pplants[i] == pplants[j]:
          print (i, pmonths[i], pplants[i]), \
                'matches with', (j, pmonths[j], pplants[j])
          alist[m] += 1
          break
    blist[m] = nextindex - startindex - alist[m]
    clist[m] = nextnextindex - nextindex - alist[m]
    # calculate turnover rate
    bints[m] = round((alist[m] + blist[m] + clist[m]) / ((2*alist[m] + blist[m] + clist[m])/2.0) - 1.0, 4)
  print "Plant Turnover rates are: " + str(bints)
  print "Total number of comparisons made: "+ str(len(bints))
  print alist
  print blist
  print clist
...
bints = calc_intturnover(months, days, years, bees, plants, noofmonths, startofmonths2)
plantturnover = calc_speciesturnover(months, days, years, bees, plants, noofmonths, startofmonths2)
