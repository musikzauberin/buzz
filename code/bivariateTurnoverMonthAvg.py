...
def plotturnovers(a, b, drya, dryb, weta, wetb, xlabel_str, ylabel_str):
  ...
  plotpath = '../results/BivariatePlots/OldCerrado/monthavg&turnover/' + plotname + '.pdf'
  pl.savefig(plotpath)
  pl.close()
...
def writenewdata(filename_str, headers, values):
  ...
  pathname = '../results/BivariatePlots/OldCerrado/monthavg&turnover/' + filename_str + '.csv'
  g = open(pathname, 'wb')
