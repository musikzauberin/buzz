...
def extract_data2(pathname):
  ...
  data1, data2 = itertools.tee(csv.reader(h))
  # check that data columns are correct
  [cheader, cyears, cmonths, sumprecips, avghumids, maxtemps, avgtemps, tempranges, tempmedians] = ([] for i in range(len(next(data1)) + 1))
  datacolumns = [cyears, cmonths, sumprecips, avghumids, maxtemps, avgtemps, tempranges, tempmedians]
  ...
  for i in datacolumns:
    cheader.append(i.pop(0))

  return cheader, cyears, cmonths, sumprecips, avghumids, maxtemps, avgtemps, tempranges, tempmedians
...
[cheader, cyears, cmonths, sumprecips, avghumids, maxtemps, avgtemps, \
tempranges, tempmedians] = extract_data2('../data/rearranged/new/OldCerradoClimaData.csv')

for measure in [sumprecips, avghumids, maxtemps, avgtemps, \
tempranges, tempmedians]:
  for x in range(len(measure)-1):
    measure[x] = round((float(measure[x]) + float(measure[x+1]))/2, 2)

for measure in [cyears, cmonths, sumprecips, avghumids, maxtemps, avgtemps, \
tempranges, tempmedians]:
  measure.pop(-1)
...
newheaders2 = 'Year, Month, InteractionTurnover, BeeTurnover, PlantTurnover, SpeciesTurnover, B_os, B_st, SumPrecips, AvgHumidity, AvgMaxTemp, AvgTemp, AvgTempRange, TempMedian'
newvalues2 = [yearlabels, monthlabels, bints, beeturnovers, plantturnovers, specturnovers, osturnovers, stturnovers, sumprecips, avghumids, maxtemps, avgtemps, \
tempranges, tempmedians]
