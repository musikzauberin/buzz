...
pl.draw()
pl.title('Climate (Average) in Cerrado (1995-1996)')
makesubplotnice()
...
# grid lines
xs = np.arange(-0.5, 11, 1)
ys = np.arange(18, 32, 2)
for x in xs:
    pl.plot([x, x], [18, 32], color='black', alpha=0.3, linestyle='-')
for y in ys:
    pl.plot([-1, 13.5], [y, y], color='black', alpha=.33, linestyle=':')
...
p2, = par1.plot(avghumids[intervals[1]:intervals[2]], marker = 'o', color = colordict['Humid'], label = 'Humidity', markeredgewidth=0.5, markeredgecolor=almost_black)
host.set_xlim(-1, 13.5)
# axes = pl.gca()
# ymin, ymax = axes.get_ylim()
# xmin, xmax = axes.get_xlim()
...
pl.draw()
pl.title('Climate (Average) in Cerrado (1996-1997)')
makesubplotnice()
...
########## fourth plot avgtemps, avgmaxtemps, avgtempranges ##########
...
host.set_ylabel(r"Temperature / $^\circ$C")
par1.set_ylabel("Temperature Range")
p1, = host.plot(avgtemps[intervals[1]:intervals[2]], marker = 'o', color = colordict['Temp'], label = 'Temperature', markeredgewidth=0.5, markeredgecolor=almost_black)
p1, = host.plot(avgmaxtemps[intervals[1]:intervals[2]], marker = 'o', color = colordict['TempMax'], label = 'Maximum Temperature', markeredgewidth=0.5, markeredgecolor=almost_black)
p2, = par1.plot(avgtempranges[intervals[1]:intervals[2]], marker = 'o', color = colordict['TempRange'], label = 'Temperature Range', markeredgewidth=0.5, markeredgecolor=almost_black)
host.set_xlim(-1, 13.5)
# axes = pl.gca()
# ymin, ymax = axes.get_ylim()
# xmin, xmax = axes.get_xlim()
...
plotpath = '../results/' + 'AvgClimateAcrossTime(Old)' + '.pdf'
pl.savefig(plotpath)
